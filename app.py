...
SAMPLE_PATH = Path(__file__).parent / "sample_repair_age.csv"
...
def render_data_quality(clean_result) -> None:
    ...
    if clean_result.rows_dropped_negative_median:
        issues.append(f"{clean_result.rows_dropped_negative_median:,} row(s) dropped for negative median repair %.")
...
def main() -> None:
    ...
    st.caption(
        "Upload a CSV or Excel table with health segment/category, vehicle age, and median repair %. "
        "The app cleans percent formats, filters categories, and plots one line per fleet category."
    )
    ...
    with st.sidebar:
        ...
        if uploaded_file is None:
            st.info("No file uploaded. Showing included sample data.")
    ...
    with kpi1:
        st.metric("Rows plotted", f"{len(plot_df):,}")
    with kpi2:
        st.metric("Categories", f"{plot_df['category'].nunique():,}")
    with kpi3:
        st.metric("Age range", f"{int(plot_df['age'].min())}–{int(plot_df['age'].max())}")
    with kpi4:
        st.metric("Peak median repair %", format_pct(float(highest_row["median_repair_pct"])))
    with kpi5:
        st.metric("Peak category / age", f"{highest_row['category']} / {int(highest_row['age'])}")
    ...
    with left:
        ...
        display_summary = category_summary.copy().sort_values("change_pp", ascending=False)
        display_summary["first_value"] = display_summary["first_value"].map(format_pct)
        display_summary["last_value"] = display_summary["last_value"].map(format_pct)
        display_summary["peak_value"] = display_summary["peak_value"].map(format_pct)
    ...
    with right:
        ...
        change_df["change_label"] = change_df["change_pp"].map(lambda x: f"{x:+.1f}")
    ...
    st.subheader("Cleaned plotted data")
    st.dataframe(
